...
export default function NotificationItem({
  ...
  /**
   * Unread is one bit of state, and it used to be drawn four times over: an
   * accent rail down the leading edge, a dot beside the title, a tint across
   * the whole row, and a heavier title. Stacked up a list of finished scans
   * read as a wall of highlighted rows, which is exactly the noise the rail was
   * meant to cut through — and the severity icon, the only mark on the row that
   * carries real information, had to compete with all of it.
   *
   * The rail and the title weight stay. The dot and the row tint go: they said
   * nothing the rail did not already say, in the same colour, twice.
   */
  const body = (
    <>
      <RowIcon icon={Icon} className={className} />
  ...
  return (
    ...
    >
      {/* The single unread mark. Matches the sidebar's active-item rail. */}
      {!data.read && (
